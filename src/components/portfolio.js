    ...
    return arrayToMap.map((project, i) => {
      return (
        <div className="col-md-4 col-sm-6 portfolio-item" key={i}>
          <a
            onClick={() => {
              this.props.selectedModal(project);
        ...
        <Modal>
          <div className="project-info">
            ...
            <img
              className="img-responsive img-thumbnail"
              alt="projectlink"
              src={this.props.modal.imageLink}
            />
            <div className="caption-full">
              <div>
                <h5 className="pull-right">
                  <em>Project Completed: {this.props.modal.completion}</em>{" "}
                </h5>
              </div>
              ...
              <table className="table table-bordered">
                <tbody>
                  <tr>{this.renderModals()}</tr>
                </tbody>
              </table>

              <a
                className="btn btn-primary github"
                href={this.props.modal.github}
              >
                See the Code
              </a>
              <a
                className="btn btn-primary website"
                href={this.props.modal.website}
              >
                See the Website
              </a>
              <p className={classNames({ hide: !this.props.modal.herokuFree })}>
                <em>
                  This website may load slowly, as it being hosted for free on
                  Heroku as a project showcase app
                </em>
              </p>
            </div>
          </div>
...
function mapDispatchToProps(dispatch) {
  ...
  return bindActionCreators(
    { openCloseModal: openCloseModal, selectedModal: selectedModal },
    dispatch
  );
}
